...
`dsh-skill-pdf` registers the `pdf` skill the same way — one immutable `bundled` candidate whose packaged `assets/` directory carries the skill body and `scripts/pdf2text.py`, a pypdf-first extraction script with a system `pdftotext` fallback. The shipped CLI declares the plugin enabled, so every session catalog carries `pdf`.
