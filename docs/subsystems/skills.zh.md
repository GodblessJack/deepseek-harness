...
`dsh-skill-pdf` 以相同方式注册 `pdf` skill——一个不可变的 `bundled` 候选项，其随包 `assets/` 目录携带 skill 正文与 `scripts/pdf2text.py`（pypdf 优先、系统 `pdftotext` 兜底的提取脚本）。交付的 CLI 将该插件声明为启用，因此每个会话目录都携带 `pdf`。
