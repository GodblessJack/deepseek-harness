...
async function scopedBench(register?: (inputTriggers: InputTriggerService) => void) {
  ...
  const release = vi.fn()
  const shell = new SessionInputShell({ actx, inputTriggers: () => controller, defaultSink: sink, commandImages: { serialize, release, unsupportedNotice: (token: string) => `${token.trim()} images-unsupported` }, commandFiles: { unsupportedNotice: (token: string) => `${token.trim()} files-unsupported` } })
