#!/usr/bin/env node
/**
 * Mount-check for the report-writer agent preset: boot the base bundle plus the
 * agent-presets roster over a scratch root under this leaf, list the roster,
 * and ensure the report-writer standing mount (`standingKeyFor` mounts the
 * subtree without starting an agent, session, or turn). Exit 0 when
 * report-writer lists healthy and mounts; exit 1 with the reason otherwise.
 *
 * Usage: node examples/grid-ops-agent/report-writer/check-preset-mount.mjs
 */
...
const scratch = mkdtempSync(join(dirname(profileFallback), '.report-writer-mount-check-'))
let exitCode = 1

try {
  writeFileSync(join(scratch, 'cordis.yml'), '# scratch profile root — an empty entry list\n[]\n')
  const patches = [
    ...(loadOptionalPatches('report-writer-check', basePatchFile) ?? []),
    {
      insert: [{
        id: 'agent-presets',
  ...
  const ctx = await boot('report-writer-check', join(scratch, 'cordis.yml'), patches, undefined, pathToFileURL(profileFallback).href)
  try {
    ...
    const preset = presets.find(p => p.id === 'report-writer')
    if (preset === undefined) throw new Error('preset report-writer is not on the roster')
    if (preset.broken !== undefined) throw new Error(`preset report-writer is broken: ${preset.broken}`)
    await roster.standingKeyFor('report-writer')
    console.log('report-writer: listed healthy and standing mount ensured')
    exitCode = 0
  } finally {
    await ctx.fiber.dispose()
  }
} catch (error) {
  console.error(`report-writer mount check failed: ${error instanceof Error ? error.stack : String(error)}`)
