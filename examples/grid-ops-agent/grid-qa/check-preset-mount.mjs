#!/usr/bin/env node
/**
 * Mount-check for the grid-qa agent preset: boot the base bundle plus the
 * agent-presets roster over a scratch root under this leaf, list the roster,
 * and ensure the grid-qa standing mount (`standingKeyFor` mounts the subtree
 * without starting an agent, session, or turn). Exit 0 when grid-qa lists
 * healthy and mounts; exit 1 with the reason otherwise.
 *
 * Usage: node examples/grid-ops-agent/grid-qa/check-preset-mount.mjs
 */
...
const scratch = mkdtempSync(join(dirname(profileFallback), '.grid-qa-mount-check-'))
let exitCode = 1

try {
  writeFileSync(join(scratch, 'cordis.yml'), '# scratch profile root — an empty entry list\n[]\n')
  const patches = [
    ...(loadOptionalPatches('grid-qa-check', basePatchFile) ?? []),
    {
      insert: [{
        id: 'agent-presets',
  ...
  const ctx = await boot('grid-qa-check', join(scratch, 'cordis.yml'), patches, undefined, pathToFileURL(profileFallback).href)
  try {
    ...
    const gridQa = presets.find(p => p.id === 'grid-qa')
    if (gridQa === undefined) throw new Error('preset grid-qa is not on the roster')
    if (gridQa.broken !== undefined) throw new Error(`preset grid-qa is broken: ${gridQa.broken}`)
    await roster.standingKeyFor('grid-qa')
    console.log('grid-qa: listed healthy and standing mount ensured')
    exitCode = 0
  } finally {
    await ctx.fiber.dispose()
  }
} catch (error) {
  console.error(`grid-qa mount check failed: ${error instanceof Error ? error.stack : String(error)}`)
